...
class Cminds_Marketplace_Model_Adminhtml_Observer {
    public function onCustomerSave($observer) {
        ...
        try {
            ...
            if(isset($postData['categories_ids']) && $postData['all_categories_ids'] && !empty($postData['categories_ids'])) {
                ...
                */
                ...
                $savedCAtegories = Mage::getModel('marketplace/categories')
                        ->getCollection()
                        ->addFilter('supplier_id', $supplierId);
                ...
                foreach($postData['categories_ids'] AS $category) {
    ...
    private function _parseUploadedCsv($supplier_id) {
        ...
        if (isset($_FILES["tablerate_file"])) {
            $changed = true;
            if(file_exists($_FILES["tablerate_file"]["name"])) {
